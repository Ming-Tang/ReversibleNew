...
//#endif
  ...
  static member ValidBlock() =
    ...
    |> Gen.map (Block.Block >> Machine.normalize)
    |> Gen.scaleSize (min 8)
    |> Arb.fromGen
...
module MachineBuilderTests =
  ...
  [<Property>]
  let ``cperm applies permutation conditionally``(b : bool, p : Perm) =
    let arr = p.ToArray() |> Array.mapi (fun i x -> hash (i, p, x) % 2 = 0)
    let ms = MachineState(MachineBuilder.cperm p)
    let pre = [| not b; b |]
    let actual = ms.Evaluate([ Array.append pre arr ])
    let expected = [| Array.append pre (if b then Perm.apply p arr else arr) |]
    if expected <> actual then
      failwithf "%A" (expected, actual)
    else
      expected = actual
